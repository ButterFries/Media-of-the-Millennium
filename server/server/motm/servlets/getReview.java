...
public class getReview implements HttpHandler
{
    private static AppDatabase db;
    private static SessionManager sm;
    private static Connection conn = null;

    public getReview(AppDatabase appDB, SessionManager appSM) {
        db = appDB;
        sm = appSM;
    ...
    public void handle(HttpExchange r) {
        ...
        try {
        	...
        	e.printStackTrace();
            System.out.println("# ERROR getReview.handle ::  " + e);
    ...
    public void handleReq(HttpExchange r, Connection conn) throws Exception {
        ...
        if (requestJSON.has("mediaID") && requestJSON.has("userType") && requestJSON.has("user")
            ...
            System.out.println("--searching review");
            ...
            responseJSON = db.get_review(conn, userID, mediaID); //exception will be forwarded up to .handle

            System.out.println("----finished searching for review");

            /*  send response  */
            try {
                responseJSON.put("error_code", 0);
                responseJSON.put("error_description", "successfully searched for review");
